...
# Configure logging at the beginning of your views.py file or before the function definition
logging.basicConfig(level=logging.DEBUG)
logger = logging.getLogger('pytube')  # Specifically capture logs from pytube
...
def process_form(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.debug(f"Processing form for URL: {url}")  # Example of adding your own log statement
            ...
            yt = YouTube(url)
            logger.debug("YouTube object created")  # Log after creating YouTube object
            # Assuming itag 22 is the one you want for 720p resolution
            stream = yt.streams.get_lowest_resolution()
            if stream:
                # Generate the filename
                logger.debug(f"Stream found: {stream}")  # Log the stream that was found
                filename = title + '.mp4'
